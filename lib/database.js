...
class Database {
        ...
        return new Promise((resolve, reject) => {
            this.db().serialize(() => {
                this.db()
                    ...
                    .run('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, phone TEXT NOT NULL, username TEXT NOT NULL, password TEXT NOT NULL, email TEXT NOT NULL, birthdate DATETIME NOT NULL, is_verified BOOLEAN DEFAULT false);', (t, err) => {
                        resolve();
                    });
            });
        });
    ...
    async verified(id) {
        return new Promise((resolve, reject) => {
            this.db().run("UPDATE user SET is_verified = TRUE WHERE id = ?", [id], (error) => {
                if(!error) {
                    resolve(true);
                } else {
                    //  Provide feedback for the error
